...
class Mellinger(Mathfunction):
  ...
  def log_nom(self, log, t):
    log.write_nom(t=t, input_acc=self.input_acc, input_Wb=self.input_Wb, P=self.trajectory.traj_pos+self.tmp_pos, V=self.trajectory.traj_vel, Euler=self.Euler_nom, Wb=self.traj_W, Euler_rate=self.Euler_rate_nom, L=np.zeros(3),VL=np.zeros(3), q=np.zeros(3), dq=np.zeros(3))
