...
class Quad_with_Cable_Suspended(Mathfunction):
  ...
  def qcsl_init(self):
    ...
    # init trajectory
    self.kp = np.array([1.5, 1.5, 4.0])
    self.kd = np.array([3.0, 3.0, 2.0])
    self.ki = np.array([0.0, 0.0, 1.0])
    self.Lpid = PIDVEC(self.kp, self.ki, self.kd, self.dt)
    self.kpL = np.array([3.0, 3.0, 4.0])
    self.kpdL = np.array([1.5, 1.5, 1.0])
  ...
  def log_nom(self, log, t):
    ...
    log.write_nom(t=t, input_acc=self.input_acc, input_Wb=self.input_Wb, P=self.trajectory.traj_L+ self.tmp_pos, V=self.trajectory.traj_dL, Euler=self.Euler_nom, Wb=self.traj_W, Euler_rate=self.Euler_rate_nom, L=self.traj_L,VL=self.traj_dL, q=self.traj_q, dq=self.traj_dq)
